...
def _model_factory(model: str) -> Callable[[], Any]:
    """mock | qwen3-8b | ollama:<model> | any local Hugging Face model path."""
    if model == "mock":
    ...
    if model.startswith("ollama:"):
        ...
        return lambda: OllamaModelAdapter(ollama_model)
...
def run(
    ...
    model: Annotated[
        str, typer.Option(help="mock | qwen3-8b | ollama:<model> | a local HF model path")
    ] = "mock",
